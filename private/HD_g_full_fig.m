...
%
% HD_full_fig is a general function of HDFT. This function generates 
% various figures related to the previously calibrated models.
%
...
if ~isfile(filename)
    msg = 'The file "HDFT_fit.mat" is not found.';
    error(msg)
end
fits = load(filename);
fields_name = string(fieldnames(fits));
fields_name(fields_name(:)=="input") = [];

for p = 1:numel(fields_name)
    lf = fits.(fields_name(p)); %loaded fit
    if fig_req(lf.type) ~= 1
        continue
    end
    
    ml = lf.type;
    HD_g_assign([],fits.input,1);
    HD_g_assign([],lf,1);
    errors = struct('MARE',MARE,'r2',r2,'rxy',rxy);
    ...
    elseif ml == 4
        ...
        title_gen = join(["ST-R model, method:", string(AQ),", $\alpha$: ",mode,", $n:$ ",mode_n]);
        title_spec = join(["ST-R model, method:", string(AQ),", $\varepsilon = ",string(strain_spec),"$"]);
    ...
    %     end
    clearvars -except tempC sym sym_corr fig_req files corr_fig fit_figs ss_figs p fits fields_name
end
end
