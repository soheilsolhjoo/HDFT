% HD_f_sinh_revisited(CK,N,S,A,PO,AQ,norm,sf,mg):
...
% function named 'HD_sinh_revisited_fit_model.m' based on the fitted
...
% -------------------------------------------------------------------------
function HD_f_sinh_revisited(varargin)
...
[mesh_alpha,mesh_n,Q,lnA] = HD_f_sinh_conventional(CK,100,0.05,[0,0],'poly9',0);
if averaging_alpha
...
if AQ == 1
    ZH          = strate_s.*exp(Q_normalizer*s5_func/R./temp_s);
...
% Export as a m-code
matlabFunction(sigma_fit,'file','HD_sinh_revisited_fit_model.m')
end
%% save workspace
if saving_f
%     Data Structure
    HDFT_fit_temp.type = 4;
    data = ["input,call,para,poly,mesh,corr";
        "ss_approx,strain,strate,temp,u_rates,u_temp,index";
        "CK,N,strain_spec,averaging,p_order,AQ";
        "Q,lnA";
        "poly_Q,poly_lnA";
    ...
    toSave = strrep("STR" + num2str(averaging) + num2str(AQ),' ','');
